    ...
    for name, dim in dim_out.items():
        ...
        if shape_info.dim_circular(on) > 0:
            conditioner = WrapPeriodic(
                conditioner, indices=shape_info.circular_indices(on)
            )
...
class GNNConditioner(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out, hidden, activation, GNN_output_dim, **kwargs):
        ...
        for field in self.on:
            if field.is_circular:
                self.labels = (
                    ["circular"] * self.shape_info[field][0] * 2
                ) + self.labels  # add the sin, cos indices on the left
        ...
        self.cart_indices_after_periodic = np.where(
            np.array(self.labels) == "cartesian"
        )[0]
        ...
        self.densenet = bg.DenseNet(
            [self.dim_dense_in, *hidden, dim_out], activation=activation
        )
        ...
        if self.attention_level == "MHA":
            ...
            self.MHA = torch.nn.MultiheadAttention(
                embed_dim=self.attention_dim, num_heads=8, batch_first=True
            )
    ...
    def forward(self, x, context=None):
        if self.use_checkpointing:
            ...
            )  ## checkpointing does not work without having input that requires grad
    ...
    def _forward(self, x, dummy_var=None, context=None):
        ...
        if self.GNN.num_evaluations == 0:
            ...
            GNN_output = (
                self.GNN._buffer
            )  # recycle the GNN output from earlier conditioner
            self.GNN.num_evaluations += 1

        if (
            self.GNN.num_evaluations == self.GNN.num_uses
        ):  # we are at the end of the generators flow.
            self.GNN.num_evaluations = (
                0  # the next conditioner call will actually call the GNN
            )
        ...
        if self.attention_level is not None and self.attention_units > 20:
            warnings.warn(
                "unsing Attention on the GNN feature vectors might crash as there are many of them."
            )
        if self.attention_level == "MHA":
            ...
            GNN_output = self.transformer_encoder(
                GNN_output.view(batchsize, -1, self.attention_dim)
            )
...
def _affine_out_dims(
    what, shape_info, transformer_kwargs={}, use_scaling=True, **kwargs
):
...
def _mixture_out_dims(
    what, shape_info, transformer_kwargs={}, num_components=8, **kwargs
):
